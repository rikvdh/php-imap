...
class ImapProtocol extends Protocol {
     ...
     * @param Response $response
     * @param-out array|string $tokens to decode
     ...
     * @param-out string $tag provide a tag otherwise an autogenerated is returned
     *
     * @return Response
     * @throws RuntimeException
     */
    ...
    public function fetch(array|string $items, array|int $from, mixed $to = null, int|string $uid = IMAP::ST_UID): Response {
        ...
        while (!$this->readLine($response, $tokens, $tag)) {
            ...
            if ($to === null && !is_array($from) && ($uid === IMAP::ST_UID ? $tokens[2][$uidKey] == $from : $tokens[0] == $from)) {
                ...
                if (!$this->readLine($response, $tokens, $tag)) // @phpstan-ignore booleanNot.alwaysTrue
    ...
    public function getUid(?int $id = null): Response {
        if (!$this->enable_uid_cache || empty($this->uid_cache)) {
            try {
                $this->setUidCache((array)$this->fetch('UID', 1, PHP_INT_MAX)->data()); // set cache for this folder
    ...
    public function store(
        array|string $flags, int $from, ?int $to = null, ?string $mode = null, int|string $uid = IMAP::ST_UID, ?string $item = null
    ): Response {
        $flags = $this->escapeList(is_array($flags) ? $flags : [$flags]);
        $set = $this->buildSet($from, $to);

        $command = $this->buildUIDCommand("STORE", $uid);
        $item = ($mode == '-' ? "-" : "+") . ($item === null ? "FLAGS" : $item) . '.SILENT';

        return $this->requestAndResponse($command, [$set, $item, $flags], true);
    }
    ...
    public function moveMessage(string $folder, $from, ?int $to = null, int|string $uid = IMAP::ST_UID): Response {
        ...
        if (!$result->boolean()) {
            ...
            $result = $this->store(['\Deleted'], $from, $to, null, $uid);
            if (!$result->boolean()) {
                return $result;
            }
            return $this->expunge();
    ...
    public function moveManyMessages(array $messages, string $folder, int|string $uid = IMAP::ST_UID): Response {
        ...
        if (!$result->boolean()) {
            ...
            foreach ($messages as $message) {
                $result = $this->store(['\Deleted'], $message, $message, null, $uid);
                if (!$result->boolean()) {
                    return $result;
                }
            }
     ...
     */
    private function buildSet(int $from, ?int $to = null): string {
        $set = (string)$from;
        if ($to !== null) {
            $set .= ':' . (string)($to == PHP_INT_MAX ? '*' : $to);
        }
        return $set;
    }
